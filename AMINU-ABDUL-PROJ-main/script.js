  ...
  tableBody.innerHTML = data.map(item => `
      <tr>
        ...
        <td>${item.years_in_office}</td>
          <td><span class="sentiment-item" data-governor="${item.governor}" data-sentiment="Education:${item.breakdown.education}\n Healthcare:${item.breakdown.healthcare}\n Infrastructure:${item.breakdown.infrastructure}">${item.sentiment}</span></td>
        <td><a href="${item.manifesto_link}" target="_blank">View</a></td>
      </tr>
    `).join("");
}

function addSentimentClickListeners() {
  document.querySelectorAll(".sentiment-item").forEach(elem => {
    elem.addEventListener("click", () => {
      const sentiment = elem.getAttribute("data-sentiment");
      const governor = elem.getAttribute("data-governor");
      alert(`Sentiment for ${governor}:\n${sentiment}`); // Replace with custom dialog if needed
    });
  });
}
